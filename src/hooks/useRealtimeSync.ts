...
type TableName = 'items' | 'categories' | 'meals' | 'meal_ideas' | 'notes' | 'expenses' | 'expense_splits' | 'expense_quotas' | 'participants' | 'bristol_entries' | 'settlements'
...
export function useRealtimeSync({ listId, onTableChange }: UseRealtimeSyncOptions) {
  ...
  useEffect(() => {
    ...
    const tables: TableName[] = ['items', 'categories', 'meals', 'meal_ideas', 'notes', 'expenses', 'expense_splits', 'expense_quotas', 'participants', 'bristol_entries', 'settlements']
